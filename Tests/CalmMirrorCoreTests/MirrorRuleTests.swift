...
final class MirrorRuleTests: XCTestCase {
    ...
    func testBlockerTitleUsesSourceNameByDefault() {
        ...
        XCTAssertEqual(rule.blockerTitle(forSourceTitle: "Tax meeting"), "[CMA] Tax meeting")
    }
    ...
    func testBlockerTitleFallsBackForEmptySourceTitle() {
        ...
        XCTAssertEqual(rule.blockerTitle(forSourceTitle: ""), "[CMA] (No title)")
        XCTAssertEqual(rule.blockerTitle(forSourceTitle: "   "), "[CMA] (No title)")
        XCTAssertEqual(rule.blockerTitle(forSourceTitle: nil), "[CMA] (No title)")
    }
    ...
    func testBlockerTitleUsesPlaceholderWhenEnabled() {
        let rule = MirrorRule(
            ...
            targetCalendarIdentifier: validTarget,
            windowDays: validWindowDays,
            blockerLabel: "[CMA] Busy",
            usePlaceholder: true,
            isEnabled: true,
            createdAt: Date(),
            updatedAt: Date()
        )
        XCTAssertEqual(rule.blockerTitle(forSourceTitle: "Tax meeting"), "[CMA] Busy")
        XCTAssertEqual(rule.blockerTitle(forSourceTitle: ""), "[CMA] Busy")
    }
